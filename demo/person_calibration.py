...
def fine_tune(subject, data, frame_processor, mon, device, gaze_network, k,
              num_finetuning_steps=1000, finetuning_learning_rate=1e-4):
    ...
    data = frame_processor.process(subject, vid_cap, mon, device, gaze_network, por_available=True, show=False)
    vid_cap.release()
    ...
    # vid_cap = cv2.VideoCapture('%s_calib.avi' % subject)
    # data = frame_processor.process(subject, vid_cap, mon, device, gaze_network, por_available=True, show=True)
    # vid_cap.release()
